...
"""
when we call AritstRepository#find_one_id_from_name with an artist name, it returns the correct artist id
"""
def test_id_from_name(db_connection):
    db_connection.seed("seeds/music_library.sql")
    repository = ArtistRepository(db_connection)
    result = repository.find_id_from_name("Pixies")
    assert result == 1
